...
async function main() {
    while (condition) {
        ...
        todos.push({ task: addTask.todo, done: false });
        console.log(chalk.bgYellow.black("\nTodo List:"));
        console.log("-----------");
        todos.forEach((item, index) => {
            console.log(`${index + 1}. ${item.done ? chalk.gray.strikethrough(item.task) : item.task}`);
        });
        console.log("-----------\n");
        condition = addTask.addMore;
    }
...
async function markTaskDone() {
    let choices = todos.map((item, index) => ({
        name: `${index + 1}. ${item.done ? chalk.gray.strikethrough(item.task) : item.task}`,
        value: index
    }));
